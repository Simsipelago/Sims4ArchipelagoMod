...
class S4CLDebugObjectMakeCleanInteraction(CommonImmediateSuperInteraction):
    ...
    def get_log_identifier(cls) -> str:
        return 's4cl_debug_make_object_clean'
    ...
    def on_test(cls, interaction_sim: Sim, interaction_target: Any, interaction_context: InteractionContext, **kwargs) -> CommonTestResult:
        ...
        if not CommonObjectStateUtils.is_dirty(interaction_target):
            cls.get_log().debug('Failed, the Object is not dirty.')
            return CommonTestResult.NONE
        cls.get_log().debug('Success, can make object clean.')
        return CommonTestResult.TRUE

    # noinspection PyMissingOrEmptyDocstring
    def on_started(self, interaction_sim: Sim, interaction_target: GameObject) -> bool:
        return CommonObjectStateUtils.make_clean(interaction_target)
